...
export default function AdminClientTabs({ clientId, routines: initialRoutines, progress: initialProgress, messages: initialMessages }: Props) {
  ...
  const handleSendMsg = async (e: React.FormEvent) => {
    ...
    const { data } = await supabase.from('messages').insert({
      client_id: clientId, content: newMsg.trim(), sender_role: 'trainer', read: false,
    }).select().single()
  ...
  return (
    <div>
      ...
      {/* ── MENSAJES ── */}
      {tab === 'messages' && (
        <div className="flex flex-col h-[500px] bg-[#141414] border border-white/10 rounded-xl overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.length === 0 ? (
              <div className="h-full flex items-center justify-center">
                <p className="text-white/30 text-sm">No hay mensajes todavía.</p>
              </div>
            ) : (
              messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex ${isTrainer ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[75%] flex flex-col gap-1 ${isTrainer ? 'items-end' : 'items-start'}`}>
                      <span className="text-white/30 text-xs px-1">{isTrainer ? 'Vos' : 'Cliente'}</span>
                      <div className={`px-4 py-2.5 rounded-2xl text-sm ${isTrainer ? 'bg-[#ffd11e] text-black rounded-br-sm' : 'bg-white/10 text-white rounded-bl-sm'}`}>
                        {msg.content}
                      </div>
          ...
          <form onSubmit={handleSendMsg} className="border-t border-white/10 p-3 flex gap-2">
            <input
              type="text"
              value={newMsg}
            ...
            />
            <button type="submit" disabled={!newMsg.trim() || sendingMsg} className="bg-[#ffd11e] text-black font-semibold px-5 py-2.5 rounded-lg hover:bg-[#e6bc1a] transition-colors disabled:opacity-40 text-sm">
              Enviar
            </button>
          </form>
        </div>
